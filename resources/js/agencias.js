window.agenciasApiHandlers = {
    async fetchAgencias(component) {
        ...
        try {
            ...
            if (component.searchAgencia)
                params.set("search", component.searchAgencia);
            ...
            if (component.ordenarPor)
                params.set("ordenarPor", component.ordenarPor);
    ...
    async fetchClientes(component) {
        ...
        try {
            ...
            component.clientes = list.map((c) => ({
                id: c.id || c.id_cliente_pk || c.id_cliente,
                nombre:
                    c.nombre ||
                    c.nombre_comercial ||
                    c.razon_social ||
                    `Cliente ${c.id || c.id_cliente_pk || ""}`,
            }));
        } catch (e) {
    ...
    async createAgencia(component) {
        ...
        const payload = {
            ...
            clientes: Array.isArray(f.clients)
                ? f.clients
                      .map(Number)
        ...
        if (!payload.nombre_agencia || !payload.nombre_agencia.trim()) {
            window.showToast &&
                window.showToast("El nombre es obligatorio", "error");
            component.__submitting = false;
            return;
        }
        if (!payload.horario_agencia) {
            window.showToast &&
                window.showToast("El horario es obligatorio", "error");
            component.__submitting = false;
            return;
        }
        if (!payload.id_direccion_fk) {
            window.showToast &&
                window.showToast("Debe seleccionar una dirección", "error");
            component.__submitting = false;
            return;
        }
        try {
            ...
            if (!res.ok) throw data;
            window.showToast &&
                window.showToast("Agencia creada exitosamente", "success");
    ...
    async updateAgencia(component) {
        ...
        try {
            ...
            if (!res.ok) throw data;
            window.showToast &&
                window.showToast("Agencia actualizada exitosamente", "success");
            component.isAgenciaModalOpen = false;
            component.formAgencia = {
                id: null,
                nombre: "",
    ...
    async deleteAgencia(component) {
        ...
        try {
            const res = await fetch(
                `/api/agencias/${component.agenciaToDelete.id}`,
                {
                    method: "DELETE",
                    headers: { Accept: "application/json" },
                    credentials: "same-origin",
            ...
            if (!res.ok) throw data;
            window.showToast &&
                window.showToast("Agencia eliminada exitosamente", "success");
