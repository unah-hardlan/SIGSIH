            ...
            try {
                ...
                return new Promise((resolve, reject) => {
                    const run = async () => {
                        ...
                        if (res.status === 401) {
                            // No reintentar si el servidor ya devolvió un código
                            // de sesión definitivo (SESSION_EXPIRED, SESSION_REMOVED_LIMIT).
                            // Un reintento con el mismo token causaría un falso SESSION_REMOVED_LIMIT
                            // porque el middleware ya procesó (y puede haber limpiado) esa sesión.
                            let shouldRetry = true;
                            try {
                                const peek = await res.clone().json().catch(() => null);
                                if (peek && peek.code) shouldRetry = false;
                            } catch (_) { }
                            if (shouldRetry) {
                        ...
                        if (res.status === 401) {
                            try {
                                ...
                                if (
                                    ...
                                    // Ignorar si el login ocurrió hace menos de 8 segundos:
                                    // son requests en vuelo del token anterior que llegan
                                    // después de que el nuevo login ya reemplazó la sesión.
                                    let suppressLogout = false;
                                    try {
                                        const ts = parseInt(localStorage.getItem('__loginTs') || '0', 10);
                                        if (ts && (Date.now() - ts) < 8000) {
                                            suppressLogout = true;
                                            localStorage.removeItem('__loginTs');
                                        }
                                    } catch (_) { }
                                    try {
                                        window.showToast &&
