    ...
    Alpine.store("assignRoles", {
        ...
        async fetchRoles() {
            ...
            try {
                ...
                this.error =
                    e && e.message ? e.message : "Error cargando roles";
            }
        },
        ...
        _rebuildRoleMaps() {
            ...
            for (const r of this.roles || []) {
                ...
                if (!this._roleIdByKey[key])
                    this._roleIdByKey[key] = String(r.id);
            }
        },
        _roleIs(key, id) {
        ...
        _idForRole(key) {
            return this._roleIdByKey[key] || null;
        },
        // Helpers de validación Cliente vs Admin/Técnico
        _isCliente(id) {
            return this._roleIs("cliente", id);
        },
        _isAdmin(id) {
            return this._roleIs("administrador", id);
        },
        _isTecnico(id) {
            return this._roleIs("tecnico", id);
        },
        _hasClienteSelected() {
            return (
                this.rolesSelected
                    .map(String)
                    .some((id) => this._isCliente(id)) ||
                this._isCliente(this.rol_principal)
            );
        },
        _hasAdminOTecnicoSelected() {
            return (
                this.rolesSelected
                    .map(String)
                    .some((id) => this._isAdmin(id) || this._isTecnico(id)) ||
                this._isAdmin(this.rol_principal) ||
                this._isTecnico(this.rol_principal)
            );
        },
        ...
        setDirection(v) {
            this.direction = v === "desc" ? "desc" : "asc";
            this.applyFiltersAndPagination();
        },
        setFilterRol(v) {
            this.filterRol = v || "";
        ...
        applyFiltersAndPagination() {
            ...
            if (this.filterRol)
                filtered = filtered.filter(
                    (u) => String(u.id_rol_fk || "") === String(this.filterRol)
                );
            // filtro por búsqueda
            if (this.q) {
        ...
        updatePagination() {
            this.items = this.paginatedUsers();
            this.meta = {
        ...
        applyCachedRoles(id) {
            ...
            if (cached.rol_principal)
                this.rol_principal = String(cached.rol_principal);
            this.setPrincipal(this.rol_principal);
            return true;
        },
        ...
        async fetchUserRoles(id, { preferCache = false, ttlMs = 30000 } = {}) {
            ...
            if (
                ...
                this.rolesSelected = (cached.roles || []).map(String);
                this.rol_principal = cached.rol_principal
                    ? String(cached.rol_principal)
                    : this.rol_principal;
                this.setPrincipal(this.rol_principal);
                return;
            }
            if (this._rolesInflight[id]) {
                ...
                if (c2) {
                    ...
                    this.rol_principal = c2.rol_principal
                        ? String(c2.rol_principal)
                        : this.rol_principal;
                    this.setPrincipal(this.rol_principal);
                }
                return;
            ...
            this._rolesInflight[id] = (async () => {
                try {
                    ...
                    const roles = Array.isArray(data?.roles)
                        ? data.roles.map((v) => String(v))
                        : [];
                    const principal = data?.rol_principal
                        ? String(data.rol_principal)
                        : "";
                    this._rolesCache[id] = {
                        roles,
        ...
        async saveAssign() {
            ...
            try {
                ...
                const r = await fetch(`${API.users}/${id}/rol`, {
                    ...
                    credentials: "same-origin",
                    body: JSON.stringify({
                        id_rol_fk: this.form.id_rol_fk
                            ? Number(this.form.id_rol_fk)
                            : null,
                    }),
                });
                if (!r.ok)
        ...
        setPrincipal(id) {
            ...
            if (id) {
                const sid = String(id);
                if (!this.rolesSelected.map(String).includes(sid))
                    this.rolesSelected.push(String(id));
            }
        ...
        _enforceClienteRule() {
            ...
            if (this._isCliente(this.rol_principal)) {
                ...
                this.rolesSelected = rs.filter(
                    (x) =>
                        !forbid.includes(x) || x === String(this.rol_principal)
                );
                ...
                if (hasAdminOTec && clienteId) {
                    this.rolesSelected = rs.filter(
                        (x) => String(x) !== String(clienteId)
                    );
                    changed = rs.length !== this.rolesSelected.length;
                    if (changed)
                        this._notify(
                            "Cliente no puede combinarse con Administrador/Técnico. Rol Cliente removido."
                        );
                }
            }
        ...
        isRoleDisabled(id) {
            ...
            const adminOTecSel = this.rolesSelected
                .map(String)
                .some((x) => this._isAdmin(x) || this._isTecnico(x));
        ...
        async saveAssignMulti() {
            ...
            const tecnicoId = this._idForRole("tecnico");
            const hasCliente =
                roles.map(String).includes(String(clienteId)) ||
                String(rol_principal) === String(clienteId);
            const hasAdminOTec =
                roles
                    .map(String)
                    .some((x) =>
                ...
                [adminId, tecnicoId]
                    .filter(Boolean)
                    .map(String)
                    .includes(String(rol_principal));
            if (hasCliente && hasAdminOTec) {
                this._notify(
                    "No se puede guardar: Cliente no puede combinarse con Administrador/Técnico.",
                    "error"
                );
                return;
            }
