...
(function initCalendarioApiHandlers() {
    ...
    window.calendarioApiHandlers = {
        async loadCatalogs(component) {
            ...
            try {
                const [agencias, estados, tipos, clientesUnified, ordenes] =
                    await Promise.all([
                ...
                const clientes = (clientesUnified || [])
                    .map((c) => {
                        let nombre;
                        if (c.tipo === "empresa") {
                            nombre =
                ...
                for (const c of clientes) {
                    if (!uniqMap[c.id]) uniqMap[c.id] = c;
                }
                ...
                if (!clientesFinal.length) {
                    window.showToast &&
                        window.showToast(
                            "No se encontraron clientes desde Clientes/Empresas. Verifique que existan registros en esas tablas.",
                            "warning"
                        );
                }
        ...
        async fetchClientesByAgencia(component, agenciaId) {
            try {
                if (!agenciaId) {
                    ...
                    if (component.formEvento)
                        component.formEvento.id_cliente_fk = "";
                    if (component.formEventoLista)
                        component.formEventoLista.id_cliente_fk = "";
                    return [];
                }
                ...
                const ensureSelectedIn = (model) => {
                    ...
                    if (!cur) return;
                    const found = clientes.some(
                        (x) => Number(x.id) === Number(cur)
                    );
                    if (!found) model.id_cliente_fk = "";
                };
                if (component.formEvento)
                    ensureSelectedIn(component.formEvento);
                if (component.formEventoLista)
                    ensureSelectedIn(component.formEventoLista);
                return clientes;
            } catch (e) {
        ...
        async fetchOrdenesByCliente(component, clienteId) {
            try {
                if (!clienteId) {
                    ...
                    if (component.formEvento)
                        component.formEvento.id_orden_servicio_fk = "";
                    if (component.formEventoLista)
                        component.formEventoLista.id_orden_servicio_fk = "";
                    return [];
                }
                ...
                const os = (list || [])
                    .map((o) => ({
                        id:
                            o.id_orden_servicio_pk ||
                            o.id ||
                            o.id_orden_servicio,
                        label:
                            o.numero_orden_servicio ||
                            o.codigo_orden ||
                            `OS-${o.id_orden_servicio_pk || o.id}`,
                    }))
                    .filter((x) => x.id);
        ...
        async fetchMonth(component) {
            ...
            try {
                ...
                params.set("per_page", "500");
                const data = await fetchJson(
                    `/api/calendario?${params.toString()}`,
                    {
                        headers: { Accept: "application/json" },
                        credentials: "same-origin",
                    }
                ...
                for (const ev of list) {
                    ...
                    const item = {
                        ...
                        tipo: tipoNombre,
                        orden: ev.id_orden_servicio_fk
                            ? `OS-${ev.id_orden_servicio_fk}`
                            : "",
                ...
                window.showToast &&
                    window.showToast("Error al cargar eventos", "error");
                component.events = {};
            } finally {
            ...
            try {
                ...
                const payload = {
                    ...
                    observaciones_calendario:
                        form.observaciones_calendario || null,
                ...
                window.showToast &&
                    window.showToast("Evento creado", "success");
                ...
                const msg =
                    ...
                    e?.error ||
                    (e?.__http
                        ? `HTTP ${e.__http.status} ${e.__http.statusText}`
                        : "") ||
        ...
        async updateEvent(component, id, form) {
            try {
                if (
                    typeof form.id_orden_servicio_fk !== "undefined" &&
                    !form.id_orden_servicio_fk
                ) {
                    window.showToast &&
                        window.showToast(
                            "Seleccione una Orden de Servicio",
                ...
                window.showToast &&
                    window.showToast("Evento actualizado", "success");
                await this.fetchMonth(component);
                return data?.data;
            } catch (e) {
                const errs = e?.errors;
        ...
        async cancelEvent(component, id, estadoCanceladoId) {
            return this.updateEvent(component, id, {
                id_estado_calendario_fk: estadoCanceladoId,
            });
        },
        ...
        async deleteEvent(component, id) {
            try {
                ...
                window.showToast &&
                    window.showToast("Evento eliminado", "success");
                await this.fetchMonth(component);
            } catch (e) {
                window.showToast &&
                    window.showToast("No se pudo eliminar el evento", "error");
            }
        },
    };
})();
