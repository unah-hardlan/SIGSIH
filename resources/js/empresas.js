        ...
        return {
            ...
            reportUrl() {
                ...
                if (this.ordenarPor) {
                    ...
                    const serverOrden = mapOrden[this.ordenarPor] || this.ordenarPor;
                    params.set('ordenar_por', serverOrden);
                }
                // Mostrar fecha (para header) y fecha de generación detallada en el cuerpo
                const now = new Date();
                params.set('fecha', now.toLocaleDateString('es-HN', { day: '2-digit', month: 'short', year: 'numeric' }));
            ...
            _applyServerDuplicateErrors(jsonOrText, payload) {
                try {
                    ...
                    if (typeof jsonOrText === 'object') {
                        ...
                        if (jsonOrText.errors && typeof jsonOrText.errors === 'object') {
                            this.errors = jsonOrText.errors;
                            return true;
                        }
                        // A veces regresa { message: '...' }
                        if (jsonOrText.message) {
                            return this._applyDuplicateFromMessage(jsonOrText.message, payload);
                        }
                    } else if (typeof jsonOrText === 'string') {
                        return this._applyDuplicateFromMessage(jsonOrText, payload);
                    }
                } catch (_) { }
                return false;
            },
            _applyDuplicateFromMessage(message, payload) {
                ...
                let matched = false;
                const push = (k, m) => { (this.errors[k] || (this.errors[k] = [])).push(m); matched = true; };
                // Coincidencias comunes
                if (/Duplicate entry/i.test(msg) || /ya ha sido (tomad|utilizad)/i.test(msg) || /already been taken/i.test(msg)) {
                    // Intenta mapear por claves comunes en el mensaje
                    if (/nombre|nombre_comercial/i.test(msg) && payload?.nombre_comercial) push('nombre_comercial', 'El nombre comercial ya existe.');
                    if (/razon|razón|razon_social/i.test(msg) && payload?.razon_social) push('razon_social', 'La razón social ya existe.');
                    if (/rtn/i.test(msg) && payload?.rtn) push('rtn', 'El RTN ya existe.');
                    // Si no detecta campo específico, asigna genérico según payload
                    if (!matched) {
                        if (payload?.nombre_comercial) push('nombre_comercial', 'Valor duplicado.');
                        if (payload?.razon_social) push('razon_social', 'Valor duplicado.');
                ...
                if (/existe|existente|taken|únic|unique/i.test(msg)) {
                    if (payload?.nombre_comercial) push('nombre_comercial', 'El nombre comercial ya existe.');
                    if (payload?.razon_social) push('razon_social', 'La razón social ya existe.');
            ...
            async fetchEmpresas() {
                ...
                try {
                    ...
                    let r = await fetch('/api/empresas-cliente?' + params.toString(), { headers: this.apiHeaders(), credentials: 'same-origin' });
                    if (r.status === 401 && window.__AUTH?.ensureToken) { await window.__AUTH.ensureToken(true); r = await fetch('/api/empresas-cliente?' + params.toString(), { headers: this.apiHeaders(), credentials: 'same-origin' }); }
            ...
            async createEmpresaCliente() {
                ...
                try {
                    const payload = {
                        ...
                        fecha_registro: this.formEmpresa.fecha_registro || new Date().toISOString().slice(0, 10),
                        estado_cliente: (this.formEmpresa.estado_cliente || 'activo').toLowerCase()
                    };
                    let r = await fetch('/api/empresas-cliente', { method: 'POST', headers: this.apiHeaders(), credentials: 'same-origin', body: JSON.stringify(payload) });
                    if (r.status === 401 && window.__AUTH?.ensureToken) { await window.__AUTH.ensureToken(true); r = await fetch('/api/empresas-cliente', { method: 'POST', headers: this.apiHeaders(), credentials: 'same-origin', body: JSON.stringify(payload) }); }
                    ...
                    this.resetForm();
                } catch (e) { this.showToast(Object.keys(this.errors).length ? 'Corrige los campos duplicados' : 'No se creó empresa', 'error'); }
            ...
            async updateEmpresaCliente() {
                ...
                try {
                    ...
                    let r = await fetch('/api/empresas-cliente/' + this.formEmpresa.id, { method: 'PUT', headers: this.apiHeaders(), credentials: 'same-origin', body: JSON.stringify(payload) });
                    if (r.status === 401 && window.__AUTH?.ensureToken) { await window.__AUTH.ensureToken(true); r = await fetch('/api/empresas-cliente/' + this.formEmpresa.id, { method: 'PUT', headers: this.apiHeaders(), credentials: 'same-origin', body: JSON.stringify(payload) }); }
                    if (r.status === 422) { const j = await r.json(); this.errors = j.errors || {}; throw new Error('Validación'); }
                    if (!r.ok) {
                        let data = null; let text = '';
                        try { text = await r.text(); data = JSON.parse(text); } catch (_) { }
                    ...
                    this.isEmpresaModalOpen = false;
                    this.resetForm();
                } catch (e) { this.showToast(Object.keys(this.errors).length ? 'Corrige los campos duplicados' : 'No se actualizó empresa', 'error'); }
                finally { this.saving = false; }
            },
            async deleteEmpresaClienteApi(id) {
                this.deleting = true;
                try {
                    let r = await fetch('/api/empresas-cliente/' + id, { method: 'DELETE', headers: this.apiHeaders(), credentials: 'same-origin' });
                    if (r.status === 401 && window.__AUTH?.ensureToken) { await window.__AUTH.ensureToken(true); r = await fetch('/api/empresas-cliente/' + id, { method: 'DELETE', headers: this.apiHeaders(), credentials: 'same-origin' }); }
                    if (!r.ok) throw new Error('Error');
            ...
            init() {
                ...
                ['lun', 'mar', 'mie', 'jue', 'vie', 'sab', 'dom'].forEach(k => {
                    this.$watch('horarioUI.dias.' + k, () => this.syncHorarioString());
                });
            ...
            sortEmpresasLocal() {
                ...
                if (campo === 'fecha_registro') {
                    ...
                    this.empresas.sort((a, b) => {
                        const ad = new Date(a.raw?.fecha_registro || a.fecha_registro || 0);
                        const bd = new Date(b.raw?.fecha_registro || b.fecha_registro || 0);
                        return bd - ad;
                    });
                    return;
                }
            ...
            submitEmpresa() {
                ...
                if (this.formEmpresa.id) { this.updateEmpresaCliente(); } else { this.createEmpresaCliente(); }
            },
            deleteEmpresa() {
                if (this.empresaToDelete) { this.deleteEmpresaClienteApi(this.empresaToDelete.id); }
            },
            resetHorarioUI() {
                this.horarioUI = { dias: { lun: false, mar: false, mie: false, jue: false, vie: false, sab: false, dom: false }, desde: '08:00', hasta: '17:00' };
            },
            diasLabels() {
                return [
                    { k: 'lun', t: 'Lun' }, { k: 'mar', t: 'Mar' }, { k: 'mie', t: 'Mié' },
                    { k: 'jue', t: 'Jue' }, { k: 'vie', t: 'Vie' }, { k: 'sab', t: 'Sáb' }, { k: 'dom', t: 'Dom' }
                ];
            },
            setDias(what) {
                const all = ['lun', 'mar', 'mie', 'jue', 'vie', 'sab', 'dom'];
                if (what === 'lv') {
                    all.forEach(k => this.horarioUI.dias[k] = ['lun', 'mar', 'mie', 'jue', 'vie'].includes(k));
                } else if (what === 'todos') {
            ...
            formatDiasCompact() {
                ...
                const label = { lun: 'Lun', mar: 'Mar', mie: 'Mié', jue: 'Jue', vie: 'Vie', sab: 'Sáb', dom: 'Dom' };
                const sel = order.filter(k => this.horarioUI.dias[k]);
                if (sel.length === 0) return '';
                let groups = []; let start = null; let prev = null;
                const pushGroup = (s, e) => { if (s === e) groups.push(label[s]); else groups.push(label[s] + '-' + label[e]); };
                sel.forEach(k => {
                    if (start === null) { start = prev = k; } else {
                        const idxPrev = order.indexOf(prev), idxCur = order.indexOf(k);
                        if (idxCur === idxPrev + 1) { prev = k; } else { pushGroup(start, prev); start = prev = k; }
                    }
                });
            ...
            syncHorarioString() {
                ...
                this.formEmpresa.horario_atencion = diasStr ? `${diasStr} ${tiempo}` : '';
            },
            parseHorarioToUI(str) {
                ...
                try {
                    ...
                    if (m) { this.horarioUI.desde = this.normHora(m[1]); this.horarioUI.hasta = this.normHora(m[2]); }
                    const dias = ['Lun', 'Mar', 'Mié', 'Jue', 'Vie', 'Sáb', 'Dom'];
                    const map = { Lun: 'lun', Mar: 'mar', 'Mié': 'mie', Jue: 'jue', Vie: 'vie', 'Sáb': 'sab', Dom: 'dom' };
                    const dayPart = str.replace(/\d{1,2}:\d{2}.*$/, '').trim();
                    if (dayPart) {
                        dayPart.split(',').map(s => s.trim()).forEach(tok => {
                            if (!tok) return;
                            const r = tok.split('-').map(x => x.trim());
                            if (r.length === 1) { const key = map[r[0]]; if (key) this.horarioUI.dias[key] = true; }
                            else if (r.length === 2) {
                                const a = dias.indexOf(r[0]); const b = dias.indexOf(r[1]);
                                if (a > -1 && b > -1 && a <= b) { for (let i = a; i <= b; i++) { this.horarioUI.dias[map[dias[i]]] = true; } }
                            }
                        });
                    }
                    this.syncHorarioString();
            ...
            validateEmpresaForm() {
                this.errors = {};
                const add = (k, m) => { (this.errors[k] || (this.errors[k] = [])).push(m); };
                if (!this.formEmpresa.nombre_comercial || !this.formEmpresa.nombre_comercial.trim()) add('nombre_comercial', 'El nombre comercial es obligatorio.');
                if ((this.formEmpresa.nombre_comercial || '').length > 150) add('nombre_comercial', 'Máximo 150 caracteres.');
                if ((this.formEmpresa.razon_social || '').length > 150) add('razon_social', 'Máximo 150 caracteres.');
                if ((this.formEmpresa.rtn || '').length > 0 && !/^[-0-9A-Za-z]{3,30}$/.test(this.formEmpresa.rtn || '')) add('rtn', 'RTN inválido, use solo números/letras y guiones (3-30).');
                if ((this.formEmpresa.descripcion_empresa || '').length > 255) add('descripcion_empresa', 'Máximo 255 caracteres.');
                if (!this.formEmpresa.fecha_registro) add('fecha_registro', 'La fecha es obligatoria.');
                // Validaciones de duplicados en cliente (rápidas). En update se excluye el propio id.
                const excludeId = this.formEmpresa.id || null;
                if (this._isDuplicateLocal('nombre_comercial', this.formEmpresa.nombre_comercial, excludeId)) add('nombre_comercial', 'El nombre comercial ya existe.');
                if (this._isDuplicateLocal('razon_social', this.formEmpresa.razon_social, excludeId)) add('razon_social', 'La razón social ya existe.');
                if (this._isDuplicateLocal('rtn', this.formEmpresa.rtn, excludeId)) add('rtn', 'El RTN ya existe.');
                const anyDay = Object.values(this.horarioUI.dias).some(Boolean);
                if (anyDay && this.horarioUI.desde >= this.horarioUI.hasta) add('horario_atencion', 'La hora inicial debe ser menor que la final.');
                this.syncHorarioString();
                if (!['activo', 'inactivo'].includes((this.formEmpresa.estado_cliente || '').toLowerCase())) add('estado_cliente', 'Estado inválido.');
