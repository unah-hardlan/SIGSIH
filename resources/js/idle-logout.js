(function () {
        ...
        try {
            ...
            if (!el || !el.textContent)
                return {
                    ...
                    warnSeconds: DEFAULT_WARNING_SECONDS,
                };
            const cfg = JSON.parse(el.textContent);
            const minutes = Math.max(
                1,
                parseInt(cfg.minutes || DEFAULT_IDLE_MINUTES, 10)
            );
            const warnSeconds = Math.max(
                5,
                parseInt(cfg.warnSeconds || DEFAULT_WARNING_SECONDS, 10)
            );
    ...
    const warnMs =
        rawWarnMs >= idleMs
            ? Math.max(5000, Math.floor(idleMs * 0.2))
            : rawWarnMs;
    ...
    function setLastActivity(ts) {
        ...
        if (bc) {
            try {
                bc.postMessage({ type: "activity", at: ts });
            } catch (_) {}
        }
    }
    ...
    function createWarning() {
        ...
        box.innerHTML =
            '<h3 style="font-weight:600;margin-bottom:0.5rem;">Sesión por expirar</h3>' +
    ...
    function startWarningCountdown(deadline) {
        ...
        const tick = () => {
            ...
            if (label)
                label.textContent = `Se cerrará en ${formatSeconds(
                    remain / 1000
                )}`;
            if (remain <= 0) {
