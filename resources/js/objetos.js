...
function normalizeMeta(payload) {
    ...
    return {
        page: 1,
        per_page: normalizeList(payload).length,
        total: normalizeList(payload).length,
        last_page: 1,
    };
}
    ...
    return {
        ...
        async init() {
            if (!hasConfiguracionAcceso()) {
                ...
                this.tipos = [];
                this.error =
                    "No tienes permisos para ver los objetos del sistema.";
                return;
            }
            this.blocked = false;
            await Promise.allSettled([this.fetchTipos(), this.fetchList(1)]);
        },
        ...
        applyFiltersAndPagination() {
            ...
            if (this.q) {
                ...
                filtered = filtered.filter(
                    (o) =>
                        (o.nombre_objeto || "").toLowerCase().includes(s) ||
        ...
        async fetchList(page = 1) {
            ...
            try {
                ...
                if (!isAbort(e)) this.error = parseErr(e);
            } finally {
                this.loading = false;
        ...
        setSearch(val) {
            this.q = val;
            this.error = "";
            if (!this.blocked) this.debouncedApplyFilters();
        },
        setTipo(val) {
            this.tipoId = val;
            this.error = "";
        ...
        tipoOptions() {
            ...
            for (const it of this.items) {
                ...
                if (t && t.id != null) {
                    const id = t.id;
                    if (!map.has(id))
                        map.set(id, { id, nombre: t.nombre || `Tipo #${id}` });
                }
            }
            if (map.size)
        ...
        async create() {
            try {
                ...
                const payload = {
                    ...
                    descripcion_objeto:
                        (this.form.descripcion_objeto || "").trim() || null,
        ...
        async remove() {
            ...
            try {
                this.loading = true;
                this.error = "";
                const r = await fetch(`${API.objetos}/${this.current.id}`, {
                    method: "DELETE",
                    headers: authHeaders(),
                    credentials: "same-origin",
                });
