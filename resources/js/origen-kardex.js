window.origenKardexApiHandlers = {
    async fetchOrigenes(component) {
        ...
        try {
            ...
            component.origenes = Array.isArray(data?.data)
                ? data.data
                : Array.isArray(data)
                ? data
                : [];
            ...
            window.showToast &&
                window.showToast("Error al cargar orígenes", "error");
        } finally {
    ...
    async submitOrigen(component) {
        ...
        if (
            component.origenes.some(
                (o) =>
                    (o.nombre_origen || "").toLowerCase() ===
                    nombre.toLowerCase()
            )
        ) {
            return (
                window.showToast &&
                window.showToast("Ya existe un origen con ese nombre", "error")
            );
        }
        try {
            ...
            component.nombre_origen = "";
            component.descripcion_origen = "";
            ...
            const validationMsg =
                err?.message ||
                (Array.isArray(err?.errors?.nombre_origen)
                    ? err.errors.nombre_origen[0]
                    : null) ||
                (Array.isArray(err?.errors?.descripcion_origen)
                    ? err.errors.descripcion_origen[0]
                    : null) ||
                (Array.isArray(err?.errors?.activo)
                    ? err.errors.activo[0]
                    : null);
            const msg =
                validationMsg || err?.error || "Error al crear el origen";
    ...
    async updateOrigen(component) {
        ...
        const payload = {
            nombre_origen: String(
                component.itemToEdit.nombre_origen || ""
            ).trim(),
            descripcion_origen: String(
                component.itemToEdit.descripcion_origen || ""
            ).trim(),
        ...
        if (
            component.origenes.some(
                (o) =>
                    (o.nombre_origen || "").toLowerCase() ===
                        payload.nombre_origen.toLowerCase() &&
                    o.id_origen_pk !== component.itemToEdit.id_origen_pk
            )
        ) {
            return (
                window.showToast &&
                window.showToast(
                    "Ya existe otro origen con ese nombre",
                    "error"
                )
            );
        }
        try {
            const resp = await fetch(
                `/api/origenes/${component.itemToEdit.id_origen_pk}`,
                {
                    method: "PUT",
                    headers: {
                        "Content-Type": "application/json",
            ...
            if (!resp.ok) throw data;
            window.showToast &&
                window.showToast("Origen actualizado", "success");
            ...
            const validationMsg =
                ...
                (Array.isArray(err?.errors?.activo)
                    ...
                    : null);
            const msg =
                validationMsg || err?.error || "Error al actualizar el origen";
            window.showToast && window.showToast(msg, "error");
        }
    },
    async deleteOrigen(component) {
        ...
        try {
            const resp = await fetch(
                `/api/origenes/${component.itemToDelete.id_origen_pk}`,
                {
                    method: "DELETE",
                    headers: { Accept: "application/json" },
                    credentials: "same-origin",
