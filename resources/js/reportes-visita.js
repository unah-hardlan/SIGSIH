window.reportesVisitaApiHandlers = {
    ...
    async fetchCatalogs(component) {
        try {
            const [tv, sr, ar, os] = await Promise.all([
                fetch("/api/tipos-visita?all=1", {
                    headers: this.headers(),
                    credentials: "same-origin",
                }),
                fetch("/api/servicios-realizados?all=1", {
                    headers: this.headers(),
                    credentials: "same-origin",
                }),
                fetch("/api/acciones-realizadas?all=1", {
                    headers: this.headers(),
                    credentials: "same-origin",
                }),
                fetch("/api/ordenes-servicio?all=1", {
                    headers: this.headers(),
                    credentials: "same-origin",
                }),
            ]);
            ...
            component.tiposVisita = Array.isArray(tvData?.data)
                ? tvData.data
                : Array.isArray(tvData)
                ? tvData
                : [];
            component.serviciosRealizados = Array.isArray(srData?.data)
                ? srData.data
                : Array.isArray(srData)
                ? srData
                : [];
            component.accionesRealizadas = Array.isArray(arData?.data)
                ? arData.data
                : Array.isArray(arData)
                ? arData
                : [];
            component.ordenesServicio = Array.isArray(osData?.data)
                ? osData.data
                : Array.isArray(osData)
                ? osData
                : [];
        } catch (e) {
            console.error("Error cargando catálogos reporte:", e);
        }
    },
    async fetchReportes(component) {
        ...
        try {
            ...
            if (component.filtroTipoVisita)
                p.set("id_tipo_visita_fk", component.filtroTipoVisita);
            if (component.filtroServicioRealizado)
                p.set(
                    "id_servicio_realizado_fk",
                    component.filtroServicioRealizado
                );
            if (component.filtroAccionRealizada)
                p.set(
                    "id_accion_realizada_fk",
                    component.filtroAccionRealizada
                );
            if (component.filtroOrdenServicio)
                p.set("id_orden_servicio_fk", component.filtroOrdenServicio);
            ...
            if (component.ordenarPor) {
                ...
                p.set(
                    "sort",
                    sortMap[component.ordenarPor] || component.ordenarPor
                );
                if (component.ordenarDirection)
                    p.set("direction", component.ordenarDirection);
            }
            ...
            component.reportes = items.map((i) => {
                const osNum =
                    i?.orden_servicio?.numero_orden_servicio ||
                    i?.numero_orden_servicio ||
                    (Array.isArray(component.ordenesServicio)
                        ? component.ordenesServicio.find(
                              (os) =>
                                  Number(os.id_orden_servicio_pk) ===
                                  Number(i.id_orden_servicio_fk)
                          )?.numero_orden_servicio
                        : null);
                const osId =
                    i?.orden_servicio?.id_orden_servicio_pk ||
                    i?.id_orden_servicio_fk;
                ...
                return {
                    ...
                    servicio_realizado:
                        i.servicio_realizado?.nombre_servicio || "",
            ...
            window.showToast &&
                window.showToast("Error al cargar reportes", "error");
        } finally {
            component.loadingReportes = false;
        }
    },
    async storeReporte(component) {
        ...
        try {
            const payload = {
                ...
                id_servicio_realizado_fk: Number(
                    component.new_id_servicio_realizado_fk
                ),
                id_accion_realizada_fk: Number(
                    component.new_id_accion_realizada_fk
                ),
                id_orden_servicio_fk: Number(
                    component.new_id_orden_servicio_fk
                ),
            };
            const resp = await fetch("/api/reportes-visita", {
                method: "POST",
                headers: this.headers(),
                credentials: "same-origin",
    ...
    async updateReporte(component) {
        ...
        try {
            const payload = {
                ...
                id_servicio_realizado_fk: Number(
                    component.edit_id_servicio_realizado_fk
                ),
                id_accion_realizada_fk: Number(
                    component.edit_id_accion_realizada_fk
                ),
                id_orden_servicio_fk: Number(
                    component.edit_id_orden_servicio_fk
                ),
            };
            const resp = await fetch(
                `/api/reportes-visita/${component.reporteToEdit.id_reportes_pk}`,
                {
                    method: "PUT",
                    headers: this.headers(),
                    credentials: "same-origin",
    ...
    async deleteReporte(component) {
        ...
        try {
            const resp = await fetch(
                `/api/reportes-visita/${component.reporteToDelete.id_reportes_pk}`,
                {
                    method: "DELETE",
                    headers: this.headers(),
                    credentials: "same-origin",
            ...
            if (!resp.ok) throw data;
            window.showToast &&
                window.showToast("Reporte eliminado", "success");
