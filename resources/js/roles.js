...
const normalizeRoleName = (s) => (s || '').toString().normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase().trim();
const HIDDEN_ROLE_NAMES = new Set(['cliente']);
const filterHiddenRoles = (list) => list.filter(role => !HIDDEN_ROLE_NAMES.has(normalizeRoleName(role?.rol || '')));
    ...
    async create() {
      try {
        ...
        if (access) {
          ...
          if (all) { access.roles = filterHiddenRoles(normalizeList(all)); }
        }
