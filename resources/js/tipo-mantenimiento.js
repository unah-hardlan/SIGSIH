window.tipoMantenimientoApiHandlers = {
    async fetchTipos(component) {
        ...
        try {
            ...
            const filtro =
                component.filtroTipoMantenimiento || component.filtro || "";
            ...
            if (component.ordenarPor) {
                const sortField =
                    component.ordenarPor === "nombre"
                        ? "tipo_mantenimiento"
                        : component.ordenarPor;
                params.set("sort", sortField);
            }
            const resp = await fetch(
                `/api/tipos-mantenimiento?${params.toString()}`,
                {
            ...
            component.items = Array.isArray(data?.data)
                ? data.data
                : Array.isArray(data)
                ? data
                : [];
            ...
            window.showToast &&
                window.showToast(
                    "Error al cargar tipos de mantenimiento",
                    "error"
                );
        } finally {
    ...
    async submitTipo(component) {
        ...
        if (
            component.items.some(
                (o) =>
                    (o.tipo_mantenimiento || "").toLowerCase() ===
                    nombre.toLowerCase()
            )
        ) {
            return (
                window.showToast &&
                window.showToast("Ya existe un tipo con ese nombre", "error")
            );
        }
        try {
            ...
            if (!resp.ok) throw data;
            window.showToast &&
                window.showToast("Tipo de mantenimiento creado", "success");
            ...
            const validationMsg =
                err?.message ||
                (Array.isArray(err?.errors?.tipo_mantenimiento)
                    ? err.errors.tipo_mantenimiento[0]
                    : null) ||
                (Array.isArray(err?.errors?.descripcion_mantenimiento)
                    ? err.errors.descripcion_mantenimiento[0]
                    : null);
    ...
    async updateTipo(component) {
        ...
        const nombre = String(
            component.edit_tipo_mantenimiento ??
                component.itemToEdit?.tipo_mantenimiento ??
                ""
        ).trim();
        const descripcion = String(
            component.edit_descripcion_mantenimiento ??
                component.itemToEdit?.descripcion_mantenimiento ??
                ""
        ).trim();
        const payload = {
            tipo_mantenimiento: nombre,
        ...
        if (
            component.items.some(
                (o) =>
                    (o.tipo_mantenimiento || "").toLowerCase() ===
                        payload.tipo_mantenimiento.toLowerCase() &&
                    o.id_tipo_mantenimiento_pk !==
                        component.itemToEdit.id_tipo_mantenimiento_pk
            )
        ) {
            return (
                window.showToast &&
                window.showToast("Ya existe otro tipo con ese nombre", "error")
            );
        }
        try {
            const resp = await fetch(
                `/api/tipos-mantenimiento/${component.itemToEdit.id_tipo_mantenimiento_pk}`,
                {
                    method: "PUT",
                    headers: {
                        "Content-Type": "application/json",
    ...
    async deleteTipo(component) {
        ...
        try {
            const resp = await fetch(
                `/api/tipos-mantenimiento/${component.itemToDelete.id}`,
                {
                    method: "DELETE",
                    headers: { Accept: "application/json" },
                    credentials: "same-origin",
