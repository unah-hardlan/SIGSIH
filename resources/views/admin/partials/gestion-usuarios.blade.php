<div x-data="usuariosCrud" x-init="init()" class="p-4 space-y-4">
            ...
            <table class="min-w-full text-sm">
                <thead>
                    <tr class="bg-gray-100">
                        ...
                        <th class="py-2 px-4 text-left nunito-bold">Correo</th>
                ...
                <tbody>
                    ...
                    <template x-for="u in users" :key="u.id">
                        <tr class="border-b hover:bg-gray-50">
                            ...
                            <td class="py-2 px-4 nunito-regular" x-text="u.correo_electronico"></td>
    ...
    <x-admin.form-modal class="nunito-bold" modalName="isModalOpen" title="Agregar Usuario" submitLabel="Guardar" formId="formCrear">
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div><label class="block text-sm nunito-bold">Nombre</label><input type="text" x-model="createForm.nombre_usuario"
                    class="mt-1 w-full border rounded px-2 py-1 nunito-regular" required></div>
            <div><label class="block text-sm nunito-bold">Usuario</label><input type="text" x-model="createForm.usuario"
                    class="mt-1 w-full border rounded px-2 py-1 nunito-regular" required></div>
            <div><label class="block text-sm nunito-bold">Correo</label><input type="email" x-model="createForm.correo_electronico"
                    class="mt-1 w-full border rounded px-2 py-1 nunito-regular" required></div>
            <div><label class="block text-sm nunito-bold">Estado</label><select x-model="createForm.estado_usuario"
                    class="mt-1 w-full border rounded px-2 py-1 nunito-regular">
                    <option value="ACTIVO">ACTIVO</option>
                    <option value="INACTIVO">INACTIVO</option>
                </select></div>
            <div class="sm:col-span-2"><label class="block text-sm nunito-bold">Contraseña</label><input type="password"
                    x-model="createForm.contrasena" minlength="8" class="mt-1 w-full border rounded px-2 py-1 nunito-regular" required>
            </div>
    ...
    <x-admin.edit-modal class="nunito-bold" modalName="isEditUserModalOpen" title="Editar Usuario" itemToEdit="userToEdit"
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div><label class="block text-sm nunito-bold">Nombre</label><input type="text" x-model="editForm.nombre_usuario"
                    class="mt-1 w-full border rounded px-2 py-1 nunito-regular" required></div>
            <div><label class="block text-sm nunito-bold">Usuario</label><input type="text" x-model="editForm.usuario"
                    class="mt-1 w-full border rounded px-2 py-1 bg-gray-100 nunito-regular" disabled></div>
            <div><label class="block text-sm nunito-bold">Correo</label><input type="email" x-model="editForm.correo_electronico"
                    class="mt-1 w-full border rounded px-2 py-1 nunito-regular" required></div>
            <div><label class="block text-sm nunito-bold">Estado</label><select x-model="editForm.estado_usuario"
                    class="mt-1 w-full border rounded px-2 py-1 nunito-regular">
                    <option value="ACTIVO">ACTIVO</option>
                    <option value="INACTIVO">INACTIVO</option>
                </select></div>
            <div class="sm:col-span-2"><label class="block text-sm nunito-bold">Nueva Contraseña (opcional)</label><input
                    type="password" x-model="editForm.contrasena" minlength="8"
                    class="mt-1 w-full border rounded px-2 py-1 nunito-regular" placeholder="Dejar en blanco"></div>
            <div class="sm:col-span-2 text-red-600 text-sm nunito-regular" x-show="formError" x-text="formError"></div>
        </div>
    </x-admin.edit-modal>
