...
}" x-init="fetchProyectos(); fetchIngresos(); fetchGastos(); fetchCatalogos();" x-effect="
...
" @keydown.escape.window="
    ...
    isGastoEditModalOpen = false;
    isGastoDeleteModalOpen = false;
" @modal-submit.window="handleModalSubmit($event)" @confirm-delete.window="handleDelete()" x-effect="if (ingresoToEdit && isIngresoEditModalOpen) {
    $nextTick(() => {
        ...
        <x-responsive-table class="bg-white dark:bg-gray-900 rounded-xl shadow-lg p-4">
            ...
            <x-slot name="actions">
                <div class="flex flex-col sm:flex-row items-center gap-2">
                    @perm(['Gestión de proyectos','Gestion de proyectos','Proyectos'], 'insercion')
                    <button @click.prevent="openNuevoProyecto()"
                            ...
                            <template x-for="(proyecto, index) in paginatedProyectos()" :key="proyecto.id_proyecto_pk">
                                <tr class="border-b border-gray-200 dark:border-gray-700 nunito-regular"
                                    ...
                                    <td class="py-2 px-4 text-gray-900 dark:text-gray-200"
                                        x-text="proyecto.nombre_proyecto"></td>
                                    <td class="py-2 px-4 text-gray-900 dark:text-gray-200"
                                        x-text="formatDate(proyecto.fecha_inicio_proyecto)"></td>
                                    <td class="py-2 px-4 text-gray-900 dark:text-gray-200"
                                        x-text="formatDate(proyecto.fecha_estimada_fin_proyecto)"></td>
                                    <td class="py-2 px-4 text-gray-900 dark:text-gray-200"
                                        x-text="formatDate(proyecto.fecha_finalizacion_proyecto)"></td>
                                    <td class="py-2 px-4 text-gray-900 dark:text-gray-200"
                                        x-text="proyecto.orden_servicio ? proyecto.orden_servicio.numero_orden_servicio : 'N/A'">
                                    </td>
                                    <td class="py-2 px-4 text-gray-900 dark:text-gray-200"
                                        x-text="proyecto.descripcion_proyecto"></td>
                                    <td class="py-2 px-4 text-gray-900 dark:text-gray-200">
                                        <span
                                            x-text="(proyecto.estado_proyecto?.nombre) || proyecto.estado_proyecto?.nombre_estado || 'Sin Estado'"></span>
                                    </td>
                                    <td class="py-2 px-4 flex gap-2">
                                        @perm(['Gestión de proyectos','Gestion de proyectos','Proyectos'],
                                        'actualizacion')
                                        <a href="#"
                                            @click.prevent="itemToEdit = JSON.parse(JSON.stringify(proyecto)); $nextTick(() => { isProyectoEditModalOpen = true; })"
                                            class="text-blue-500 hover:text-blue-700" title="Editar"><i
                                                class="fas fa-edit"></i></a>
                                        @else
                                        <span class="text-gray-400 cursor-not-allowed"
                                            title="Sin permiso para editar"><i class="fas fa-edit"></i></span>
                                        @endperm
                                        @perm(['Gestión de proyectos','Gestion de proyectos','Proyectos'],
                                        'eliminacion')
                                        <a href="#"
                                            @click.prevent="isProyectoDeleteModalOpen = true; itemToDelete = { id_proyecto_pk: proyecto.id_proyecto_pk, nombre: proyecto.nombre_proyecto }"
                                            class="text-red-500 hover:text-red-700" title="Eliminar"><i
                                                class="fas fa-trash"></i></a>
                                        @else
                                        <span class="text-gray-400 cursor-not-allowed"
                                            title="Sin permiso para eliminar"><i class="fas fa-trash"></i></span>
                                        @endperm
                                    </td>
                                </tr>
                            </template>
                        ...
                        <div
                            ...
                            <div>
                                <h3 class="font-semibold text-gray-900 dark:text-gray-200 nunito-bold"
                                    x-text="proyecto.nombre_proyecto"></h3>
                            </div>
                            <p class="text-sm text-gray-600 dark:text-gray-400 nunito-regular"
                                x-text="proyecto.descripcion_proyecto"></p>
                            <p class="text-sm text-gray-600 dark:text-gray-400 nunito-regular"
                                x-text="'Fecha inicio: ' + formatDate(proyecto.fecha_inicio_proyecto)"></p>
                            <div class="flex justify-end gap-2 pt-3 border-t border-gray-200 dark:border-gray-700">
                                @perm(['Gestión de proyectos','Gestion de proyectos','Proyectos'], 'actualizacion')
                                <button
                                    @click.prevent="itemToEdit = JSON.parse(JSON.stringify(proyecto)); $nextTick(() => { isProyectoEditModalOpen = true; })"
                                ...
                                @endperm
                                @perm(['Gestión de proyectos','Gestion de proyectos','Proyectos'], 'eliminacion')
                                <button
                                    @click.prevent="isProyectoDeleteModalOpen = true; itemToDelete = { id_proyecto_pk: proyecto.id_proyecto_pk, nombre: proyecto.nombre_proyecto }"
            ...
            <div class="mb-2">
                <span
                    ...
                    <strong class="font-medium mx-1 text-gray-900 dark:text-white"
                        x-text="(currentPageProyectos - 1) * perPageProyectos + 1"></strong>
                    a
                    <strong class="font-medium mx-1 text-gray-900 dark:text-white"
                        x-text="Math.min(currentPageProyectos * perPageProyectos, numbersProyectos.length)"></strong>
                    de
                    <strong class="font-medium mx-1 text-gray-900 dark:text-white"
                        x-text="numbersProyectos.length"></strong>
                    resultados
                </span>
            </div>
            <div
    ...
    <div x-show="tab==='movimientos'" x-cloak class="space-y-8">
        ...
        <x-responsive-table class="bg-white dark:bg-gray-900 rounded-xl shadow-lg p-4">
            ...
            <x-slot name="actions">
                @perm(['Gestión de proyectos','Gestion de proyectos','Proyectos'], 'insercion')
                <button @click="isIngresoModalOpen = true"
                    class="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-lg nunito-regular text-sm">
                    Agregar Ingreso
                </button>
                @else
                <button disabled title="Sin permiso para crear"
                    class="bg-gray-300 text-gray-600 px-4 py-2 rounded-lg nunito-regular text-sm cursor-not-allowed">
                    Agregar Ingreso
                </button>
                @endperm
            </x-slot>
                            ...
                            <template x-for="(ingreso, index) in paginatedIngresos()" :key="ingreso.id_ingresos_pk">
                                <tr class="border-b border-gray-200 dark:border-gray-700 nunito-regular"
                                    ...
                                    <td class="py-2 px-4 flex gap-2">
                                        ...
                                        'actualizacion')
                                        <a href="#"
                                            @click.prevent="ingresoToEdit = JSON.parse(JSON.stringify(ingreso)); $nextTick(() => { isIngresoEditModalOpen = true; })"
                                            class="text-blue-500 hover:text-blue-700" title="Editar"><i
                                                class="fas fa-edit"></i></a>
                                        @else
                                        <span class="text-gray-400 cursor-not-allowed"
                                        ...
                                        'eliminacion')
                                        <a href="#"
                                            @click.prevent="isIngresoDeleteModalOpen = true; ingresoToDelete = { id_ingresos_pk: ingreso.id_ingresos_pk, nombre: ingreso.nombre_ingreso }"
                                            class="text-red-500 hover:text-red-700" title="Eliminar"><i
                                                class="fas fa-trash"></i></a>
                                        @else
                                        <span class="text-gray-400 cursor-not-allowed"
                ...
                <template x-if="!loadingIngresos && ingresos.length > 0">
                    <template x-for="ingreso in paginatedIngresos()" :key="ingreso.id_ingresos_pk">
                        <div
                            ...
                            <div>
                                <h3 class="font-semibold text-gray-900 dark:text-gray-200 nunito-bold"
                                    x-text="ingreso.nombre_ingreso"></h3>
                            </div>
                            <p class="text-sm text-gray-600 dark:text-gray-400 nunito-regular"
                                x-text="'Proyecto: ' + (ingreso.proyecto ? ingreso.proyecto.nombre_proyecto : 'N/A')">
                            </p>
                            <p class="text-sm text-gray-600 dark:text-gray-400 nunito-regular"
                                x-text="'Fecha: ' + formatDate(ingreso.fecha_ingreso)"></p>
                            <p class="text-sm text-gray-600 dark:text-gray-400 nunito-regular"
                                x-text="'Monto: ' + new Intl.NumberFormat('es-HN', { style: 'currency', currency: 'HNL' }).format(ingreso.monto_ingreso)">
                            </p>
                            <div class="flex justify-end gap-2 pt-3 border-t border-gray-200 dark:border-gray-700">
                                @perm(['Gestión de proyectos','Gestion de proyectos','Proyectos'], 'actualizacion')
                                <button
        ...
        <div x-show="numbersIngresos.length > perPageIngresos"
            ...
            <div class="mb-2">
                <span
                    ...
                    Mostrando
                    <strong class="font-medium mx-1 text-gray-900 dark:text-white"
                        x-text="(currentPageIngresos - 1) * perPageIngresos + 1"></strong>
                    a
                    <strong class="font-medium mx-1 text-gray-900 dark:text-white"
                        x-text="Math.min(currentPageIngresos * perPageIngresos, numbersIngresos.length)"></strong>
                    de
                    <strong class="font-medium mx-1 text-gray-900 dark:text-white"
                        x-text="numbersIngresos.length"></strong>
                    resultados
                </span>
            </div>
            <div
        ...
        <x-responsive-table class="bg-white dark:bg-gray-900 rounded-xl shadow-lg p-4">
            ...
            <x-slot name="actions">
                @perm(['Gestión de proyectos','Gestion de proyectos','Proyectos'], 'insercion')
                <button @click="isGastoModalOpen = true"
                    class="bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-lg nunito-regular text-sm">
                    Agregar Gasto
                </button>
                @else
                <button disabled title="Sin permiso para crear"
                            ...
                            <template x-for="(gasto, index) in paginatedGastos()" :key="gasto.id_gasto_pk">
                                <tr class="border-b border-gray-200 dark:border-gray-700 nunito-regular"
                                    ...
                                    <td class="py-2 px-4 flex gap-2">
                                        ...
                                        'actualizacion')
                                        <a href="#"
                                            @click.prevent="gastoToEdit = JSON.parse(JSON.stringify(gasto)); $nextTick(() => { isGastoEditModalOpen = true; })"
                                            class="text-blue-500 hover:text-blue-700" title="Editar"><i
                                                class="fas fa-edit"></i></a>
                                        @else
                                        <span class="text-gray-400 cursor-not-allowed"
                                        ...
                                        'eliminacion')
                                        <a href="#"
                                            @click.prevent="isGastoDeleteModalOpen = true; gastoToDelete = { id_gasto_pk: gasto.id_gasto_pk, nombre: gasto.nombre }"
                                            class="text-red-500 hover:text-red-700" title="Eliminar"><i
                                                class="fas fa-trash"></i></a>
                                        @else
                                        <span class="text-gray-400 cursor-not-allowed"
                ...
                <template x-if="!loadingGastos && gastos.length > 0">
                    <template x-for="gasto in paginatedGastos()" :key="gasto.id_gasto_pk">
                        <div
                            ...
                            <div>
                                <h3 class="font-semibold text-gray-900 dark:text-gray-200 nunito-bold"
                                    x-text="gasto.nombre"></h3>
                            </div>
                            <p class="text-sm text-gray-600 dark:text-gray-400 nunito-regular"
                                x-text="'Proyecto: ' + (gasto.proyecto ? gasto.proyecto.nombre_proyecto : 'N/A')"></p>
                            <p class="text-sm text-gray-600 dark:text-gray-400 nunito-regular"
                                x-text="'Fecha: ' + formatDate(gasto.fecha)"></p>
                            <p class="text-sm text-gray-600 dark:text-gray-400 nunito-regular"
                                x-text="'Monto: ' + new Intl.NumberFormat('es-HN', { style: 'currency', currency: 'HNL' }).format(gasto.monto)">
                            </p>
                            <div class="flex justify-end gap-2 pt-3 border-t border-gray-200 dark:border-gray-700">
                                @perm(['Gestión de proyectos','Gestion de proyectos','Proyectos'], 'actualizacion')
                                <button
        ...
        <div x-show="numbersGastos.length > perPageGastos"
            ...
            <div class="mb-2">
                <span
                    ...
                    Mostrando
                    <strong class="font-medium mx-1 text-gray-900 dark:text-white"
                        x-text="(currentPageGastos - 1) * perPageGastos + 1"></strong>
                    a
                    <strong class="font-medium mx-1 text-gray-900 dark:text-white"
                        x-text="Math.min(currentPageGastos * perPageGastos, numbersGastos.length)"></strong>
                    de
                    <strong class="font-medium mx-1 text-gray-900 dark:text-white"
                        x-text="numbersGastos.length"></strong>
                    resultados
                </span>
            </div>
            <div
    ...
    <div>
        ...
        <x-admin.form-modal modalName="isProyectoModalOpen" title="Nuevo Proyecto" submitLabel="Guardar Proyecto"
            formId="formProyecto" maxWidth="max-w-4xl">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <small x-show="!errors.nombre_proyecto" class="text-xs text-gray-500 block mt-1"
                        :class="formProyecto._touched && formProyecto._touched.nombre_proyecto && (nombre_proyecto === '' || nombre_proyecto.length >= 100) ? 'text-red-500' : ''">Requerido.
                ...
                <div>
                    ...
                    <select x-model="id_orden_servicio_fk" required
                        ...
                        <template x-for="os in catalogoOrdenesServicio" :key="os.id_orden_servicio_pk">
                            <option :value="os.id_orden_servicio_pk"
                                x-text="os.codigo_orden || os.numero_orden_servicio || os.nombre_orden || 'OS-' + os.id_orden_servicio_pk">
                            </option>
                        </template>
                    </select>
                    ...
                    <small x-show="!errors.id_orden_servicio_fk" class="text-xs text-gray-500 block mt-1"
                        :class="formProyecto._touched.id_orden_servicio_fk && !id_orden_servicio_fk ? 'text-red-500' : ''">Requerido.</small>
                </div>
                <div>
                ...
                <div>
                    ...
                    <small x-show="!errors.id_estado_proyecto_fk" class="text-xs text-gray-500 block mt-1"
                        :class="formProyecto._touched.id_estado_proyecto_fk && !id_estado_proyecto_fk ? 'text-red-500' : ''">Requerido.</small>
                </div>
                <div class="md:col-span-2">
                    ...
                    <small x-show="!errors.descripcion_proyecto" class="text-xs text-gray-500 block mt-1"
                        :class="formProyecto._touched && formProyecto._touched.descripcion_proyecto && descripcion_proyecto.length >= 500 ? 'text-red-500' : ''">Máximo
            ...
            <template x-if="itemToEdit">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4" x-effect="if (itemToEdit && isProyectoEditModalOpen) { $nextTick(() => {
                    ...
                    <div>
                        ...
                        <small x-show="!errors.nombre_proyecto" class="text-xs text-gray-500 block mt-1"
                            :class="itemToEdit._touched && itemToEdit._touched.nombre_proyecto && (itemToEdit.nombre_proyecto === '' || itemToEdit.nombre_proyecto.length >= 100) ? 'text-red-500' : ''">Requerido.
                            Máximo 100 caracteres.</small>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 nunito-bold">Orden de Servicio</label>
                        <select x-model="itemToEdit.id_orden_servicio_fk" required
                            ...
                            <template x-for="os in catalogoOrdenesServicio" :key="os.id_orden_servicio_pk">
                                <option :value="os.id_orden_servicio_pk"
                                    x-text="os.codigo_orden || os.numero_orden_servicio || os.nombre_orden || 'OS-' + os.id_orden_servicio_pk"
                                    :selected="itemToEdit && (os.id_orden_servicio_pk == (itemToEdit.id_orden_servicio_fk || itemToEdit.orden_servicio?.id_orden_servicio_pk))">
                                </option>
                            </template>
                        </select>
                        <template x-if="errors.id_orden_servicio_fk">
                        ...
                        <small x-show="!errors.id_orden_servicio_fk" class="text-xs text-gray-500 block mt-1"
                            :class="itemToEdit._touched && itemToEdit._touched.id_orden_servicio_fk && !itemToEdit.id_orden_servicio_fk ? 'text-red-500' : ''">Requerido.</small>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 nunito-bold">Fecha de Inicio</label>
                    ...
                    <div>
                        ...
                        <select x-model="itemToEdit.id_estado_proyecto_fk" required
                            ...
                            <option value="">Seleccione...</option>
                            <template x-for="estado in catalogoEstadosProyecto" :key="estado.id_estado_proyecto_pk">
                                <option :value="estado.id_estado_proyecto_pk" x-text="estado.nombre"
                                    :selected="itemToEdit && (estado.id_estado_proyecto_pk == (itemToEdit.id_estado_proyecto_fk || itemToEdit.estado_proyecto?.id_estado_proyecto_pk))">
                                </option>
                            </template>
                        </select>
                        <template x-if="errors.id_estado_proyecto_fk">
                        ...
                        <small x-show="!errors.id_estado_proyecto_fk" class="text-xs text-gray-500 block mt-1"
                            :class="itemToEdit._touched && itemToEdit._touched.id_estado_proyecto_fk && !itemToEdit.id_estado_proyecto_fk ? 'text-red-500' : ''">Requerido.</small>
                    </div>
                    <div class="md:col-span-2">
                        <label class="block text-sm font-medium text-gray-700 nunito-bold">Descripción</label>
                        ...
                        <small x-show="!errors.descripcion_proyecto" class="text-xs text-gray-500 block mt-1"
                            :class="itemToEdit._touched && itemToEdit._touched.descripcion_proyecto && itemToEdit.descripcion_proyecto.length > 500 ? 'text-red-500' : ''">Máximo
                            500 caracteres.</small>
                    </div>
                </div>
            </template>
        ...
        <x-admin.form-modal modalName="isIngresoModalOpen" title="Nuevo Ingreso" submitLabel="Guardar Ingreso"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <small x-show="!errors.id_proyecto_fk_ingreso" class="text-xs text-gray-500 block mt-1"
                        :class="formIngreso._touched.id_proyecto_fk_ingreso && !id_proyecto_fk_ingreso ? 'text-red-500' : ''">Requerido.</small>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 nunito-bold">Nombre</label>
                    ...
                    <small x-show="!errors.nombre_ingreso" class="text-xs text-gray-500 block mt-1"
                        :class="formIngreso._touched && formIngreso._touched.nombre_ingreso && (nombre_ingreso === '' || nombre_ingreso.length >= 100) ? 'text-red-500' : ''">Requerido.
                        Máximo 100 caracteres.</small>
                </div>
                <div>
                    ...
                    <small x-show="!errors.fecha_ingreso" class="text-xs text-gray-500 block mt-1"
                        :class="formIngreso._touched.fecha_ingreso && !fecha_ingreso ? 'text-red-500' : ''">Requerido.</small>
                </div>
                <div>
                    ...
                    <small x-show="!errors.monto_ingreso" class="text-xs text-gray-500 block mt-1"
                        :class="formIngreso._touched.monto_ingreso && !monto_ingreso ? 'text-red-500' : ''">Requerido.</small>
                </div>
                <div class="md:col-span-2">
                    ...
                    <select x-model="id_categoria_fk_ingreso" required
                        ...
                        <option value="">Seleccione...</option>
                        <template
                            x-for="cat in catalogoCategorias.filter(c => !c.tipo_categoria || c.tipo_categoria.toLowerCase() === 'ingreso')"
                            :key="cat.id_categoria_pk">
                    ...
                    <small x-show="!errors.id_categoria_fk_ingreso" class="text-xs text-gray-500 block mt-1"
                        :class="formIngreso._touched.id_categoria_fk_ingreso && !id_categoria_fk_ingreso ? 'text-red-500' : ''">Requerido.</small>
                </div>
                <div class="md:col-span-2">
                    <label class="block text-sm font-medium text-gray-700 nunito-bold">Descripción</label>
                    ...
                    <small x-show="!errors.descripcion_ingreso" class="text-xs text-gray-500 block mt-1"
                        :class="formIngreso._touched && formIngreso._touched.descripcion_ingreso && descripcion_ingreso.length >= 250 ? 'text-red-500' : ''">Máximo
                        250 caracteres.</small>
                </div>
            </div>
        </x-admin.form-modal>
        ...
        <x-admin.edit-modal modalName="isIngresoEditModalOpen" title="Editar Ingreso" formId="formEditIngreso"
            ...
            <template x-if="ingresoToEdit">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-medium text-gray-700 nunito-bold">Proyecto</label>
                        <select x-model="ingresoToEdit.id_proyecto_fk" x-bind:value="ingresoToEdit.id_proyecto_fk"
                            required
                            ...
                            <option value="">Seleccione...</option>
                            <template x-for="proyecto in catalogoProyectos" :key="proyecto.id_proyecto_pk">
                                <option :value="proyecto.id_proyecto_pk" x-text="proyecto.nombre_proyecto"
                                    :selected="ingresoToEdit && (proyecto.id_proyecto_pk == (ingresoToEdit.id_proyecto_fk || ingresoToEdit.proyecto?.id_proyecto_pk))">
                                </option>
                            </template>
                        </select>
                        ...
                        <small x-show="!errors.id_proyecto_fk" class="text-xs text-gray-500 block mt-1"
                            :class="ingresoToEdit._touched && ingresoToEdit._touched.id_proyecto_fk && !ingresoToEdit.id_proyecto_fk ? 'text-red-500' : ''">Requerido.</small>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 nunito-bold">Nombre</label>
                        ...
                        <small x-show="!errors.nombre_ingreso" class="text-xs text-gray-500 block mt-1"
                            :class="ingresoToEdit._touched && ingresoToEdit._touched.nombre_ingreso && (ingresoToEdit.nombre_ingreso === '' || ingresoToEdit.nombre_ingreso.length >= 100) ? 'text-red-500' : ''">Requerido.
                            Máximo 100 caracteres.</small>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 nunito-bold">Fecha</label>
                        ...
                        <small x-show="!errors.fecha_ingreso" class="text-xs text-gray-500 block mt-1"
                            :class="ingresoToEdit._touched && ingresoToEdit._touched.fecha_ingreso && !ingresoToEdit.fecha_ingreso ? 'text-red-500' : ''">Requerido.</small>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 nunito-bold">Monto</label>
                        <input type="text" inputmode="decimal" autocomplete="off" required
                        ...
                        <small x-show="!errors.monto_ingreso" class="text-xs text-gray-500 block mt-1"
                            :class="ingresoToEdit._touched && ingresoToEdit._touched.monto_ingreso && !ingresoToEdit.monto_ingreso ? 'text-red-500' : ''">Requerido.</small>
                    </div>
                    <div class="md:col-span-2">
                        <label class="block text-sm font-medium text-gray-700 nunito-bold">Categoría</label>
                        <select x-model="ingresoToEdit.id_categoria_fk" x-bind:value="ingresoToEdit.id_categoria_fk"
                            required
                            ...
                            <template
                                x-for="cat in catalogoCategorias.filter(c => !c.tipo_categoria || c.tipo_categoria.toLowerCase() === 'ingreso')"
                                :key="cat.id_categoria_pk">
                                <option :value="cat.id_categoria_pk" x-text="cat.nombre_categoria"
                                    :selected="ingresoToEdit && (cat.id_categoria_pk == (ingresoToEdit.id_categoria_fk || ingresoToEdit.categoria?.id_categoria_pk))">
                                </option>
                            </template>
                        </select>
                        ...
                        <small x-show="!errors.id_categoria_fk" class="text-xs text-gray-500 block mt-1"
                            :class="ingresoToEdit._touched && ingresoToEdit._touched.id_categoria_fk && !ingresoToEdit.id_categoria_fk ? 'text-red-500' : ''">Requerido.</small>
                    </div>
                    <div class="md:col-span-2">
                        <label class="block text-sm font-medium text-gray-700 nunito-bold">Descripción</label>
                        ...
                        <small x-show="!errors.descripcion_ingreso" class="text-xs text-gray-500 block mt-1"
                            :class="ingresoToEdit._touched && ingresoToEdit._touched.descripcion_ingreso && ingresoToEdit.descripcion_ingreso.length >= 250 ? 'text-red-500' : ''">Máximo
                            250 caracteres.</small>
                    </div>
                </div>
            </template>
        ...
        <x-admin.form-modal modalName="isGastoModalOpen" title="Nuevo Gasto" submitLabel="Guardar Gasto"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <small x-show="!errors.id_proyecto_fk_gasto" class="text-xs text-gray-500 block mt-1"
                        :class="formGasto._touched.id_proyecto_fk_gasto && !id_proyecto_fk_gasto ? 'text-red-500' : ''">Requerido.</small>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 nunito-bold">Nombre</label>
                    <input type="text" x-model="nombre_gasto" required maxlength="100"
                        @input="formGasto._touched.nombre_gasto = true" @blur="formGasto._touched.nombre_gasto = true"
                    ...
                    <small x-show="!errors.nombre_gasto" class="text-xs text-gray-500 block mt-1"
                        :class="formGasto._touched && formGasto._touched.nombre_gasto && (nombre_gasto === '' || nombre_gasto.length >= 100) ? 'text-red-500' : ''">Requerido.
                        Máximo 100 caracteres.</small>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 nunito-bold">Fecha</label>
                    <input type="date" x-model="fecha_gasto" required @change="formGasto._touched.fecha_gasto = true"
                    ...
                    <small x-show="!errors.fecha_gasto" class="text-xs text-gray-500 block mt-1"
                        :class="formGasto._touched.fecha_gasto && !fecha_gasto ? 'text-red-500' : ''">Requerido.</small>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 nunito-bold">Monto</label>
                    <input type="text" inputmode="decimal" autocomplete="off" required
                    ...
                    <small x-show="!errors.monto_gasto" class="text-xs text-gray-500 block mt-1"
                        :class="formGasto._touched.monto_gasto && !monto_gasto ? 'text-red-500' : ''">Requerido.</small>
                </div>
                <div class="md:col-span-2">
                    <label class="block text-sm font-medium text-gray-700 nunito-bold">Categoría</label>
                    <select x-model="id_categoria_fk_gasto" required
                        ...
                        <option value="">Seleccione...</option>
                        <template
                            x-for="cat in catalogoCategorias.filter(c => c.tipo_categoria && c.tipo_categoria.toLowerCase() === 'gasto')"
                            :key="cat.id_categoria_pk">
                            <option :value="cat.id_categoria_pk" x-text="cat.nombre_categoria"></option>
                        </template>
                    </select>
                    ...
                    <small x-show="!errors.id_categoria_fk_gasto" class="text-xs text-gray-500 block mt-1"
                        :class="formGasto._touched.id_categoria_fk_gasto && !id_categoria_fk_gasto ? 'text-red-500' : ''">Requerido.</small>
                </div>
                <div class="md:col-span-2">
                    <label class="block text-sm font-medium text-gray-700 nunito-bold">Descripción</label>
                    ...
                    <small x-show="!errors.descripcion_gasto" class="text-xs text-gray-500 block mt-1"
                        :class="formGasto._touched && formGasto._touched.descripcion_gasto && descripcion_gasto.length >= 250 ? 'text-red-500' : ''">Máximo
                        250 caracteres.</small>
                </div>
            </div>
        </x-admin.form-modal>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4" x-effect="if (gastoToEdit && isGastoEditModalOpen) { $nextTick(() => { gastoToEdit.id_proyecto_fk = gastoToEdit.proyecto?.id_proyecto_pk || ''; gastoToEdit.id_categoria_fk = gastoToEdit.categoria?.id_categoria_pk || ''; // Normalizar fecha para <input type=date>
                    ...
                    <div>
                        ...
                        <select x-model="gastoToEdit.id_proyecto_fk" required
                            ...
                            <template x-for="proyecto in catalogoProyectos" :key="proyecto.id_proyecto_pk">
                                <option :value="proyecto.id_proyecto_pk" x-text="proyecto.nombre_proyecto"
                                    :selected="gastoToEdit && (proyecto.id_proyecto_pk == (gastoToEdit.id_proyecto_fk || gastoToEdit.proyecto?.id_proyecto_pk))">
                                </option>
                            </template>
                        </select>
                        <template x-if="errors.id_proyecto_fk">
                        ...
                        <small x-show="!errors.id_proyecto_fk" class="text-xs text-gray-500 block mt-1"
                            :class="gastoToEdit._touched && gastoToEdit._touched.id_proyecto_fk && !gastoToEdit.id_proyecto_fk ? 'text-red-500' : ''">Requerido.</small>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 nunito-bold">Nombre</label>
                        ...
                        <small x-show="!errors.nombre" class="text-xs text-gray-500 block mt-1"
                            :class="gastoToEdit._touched && gastoToEdit._touched.nombre && (gastoToEdit.nombre === '' || gastoToEdit.nombre.length >= 100) ? 'text-red-500' : ''">Requerido.
                            Máximo 100 caracteres.</small>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 nunito-bold">Fecha</label>
                        ...
                        <small x-show="!errors.fecha" class="text-xs text-gray-500 block mt-1"
                            :class="gastoToEdit._touched && gastoToEdit._touched.fecha && !gastoToEdit.fecha ? 'text-red-500' : ''">Requerido.</small>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 nunito-bold">Monto</label>
                        <input type="text" inputmode="decimal" autocomplete="off" required
                        ...
                        <small x-show="!errors.monto" class="text-xs text-gray-500 block mt-1"
                            :class="gastoToEdit._touched && gastoToEdit._touched.monto && !gastoToEdit.monto ? 'text-red-500' : ''">Requerido.</small>
                    </div>
                    <div class="md:col-span-2">
                        <label class="block text-sm font-medium text-gray-700 nunito-bold">Categoría</label>
                        <select x-model="gastoToEdit.id_categoria_fk" required
                            ...
                            <template
                                ...
                                :key="cat.id_categoria_pk">
                                <option :value="cat.id_categoria_pk" x-text="cat.nombre_categoria"
                                    :selected="gastoToEdit && (cat.id_categoria_pk == (gastoToEdit.id_categoria_fk || gastoToEdit.categoria?.id_categoria_pk))">
                                </option>
                            </template>
                        </select>
                        <template x-if="errors.id_categoria_fk">
                        ...
                        <small x-show="!errors.id_categoria_fk" class="text-xs text-gray-500 block mt-1"
                            :class="gastoToEdit._touched && gastoToEdit._touched.id_categoria_fk && !gastoToEdit.id_categoria_fk ? 'text-red-500' : ''">Requerido.</small>
                    </div>
                    <div class="md:col-span-2">
                        <label class="block text-sm font-medium text-gray-700 nunito-bold">Descripción</label>
                        ...
                        <small x-show="!errors.descripcion" class="text-xs text-gray-500 block mt-1"
                            :class="gastoToEdit._touched && gastoToEdit._touched.descripcion && gastoToEdit.descripcion.length >= 250 ? 'text-red-500' : ''">Máximo
                            250 caracteres.</small>
                    </div>
                </div>
            </template>
        ...
        <x-admin.confirmation-modal modalName="isGastoDeleteModalOpen" itemToDelete="gastoToDelete"
