<div x-data="gestionSolicitudes()" x-init="init()" @include('partials.persist-tab', ['tabKey'=>
    ...
    <div x-show="tab==='solicitudes'">
        <x-responsive-table class="bg-white dark:bg-gray-900 rounded-xl shadow-lg p-4">
                        ...
                        <template x-if="loadingSolicitudes">
                            <tr>
                                <td colspan="7" class="py-8 text-center text-gray-500 nunito-regular"><i
                                        class="fas fa-spinner fa-spin mr-2"></i> Cargando solicitudes...</td>
                            </tr>
                        </template>
                        <template x-if="!loadingSolicitudes && filteredSolicitudes().length === 0">
                            <tr>
                                <td colspan="7" class="py-8 text-center text-gray-500 nunito-regular">No se encontraron
                                    solicitudes.</td>
                            </tr>
                        </template>
            ...
            <x-slot name="cards">
                <template x-if="loadingSolicitudes">
                    <div class="p-8 text-center text-gray-500"><i class="fas fa-spinner fa-spin mr-2"></i> Cargando
                        solicitudes...</div>
                </template>
                <template x-if="!loadingSolicitudes && filteredSolicitudes().length === 0">
        ...
        <x-responsive-table class="bg-white dark:bg-gray-900 rounded-xl shadow-lg p-4">
            ...
            <x-slot name="table">
                <table class="min-w-full text-sm bg-white dark:bg-gray-900 rounded-lg overflow-hidden border-collapse">
                    <thead class="bg-gray-100 dark:bg-gray-700 nunito-bold">
                        <tr>
                            <th class="py-2 px-4 text-left text-gray-900 dark:text-gray-200 nunito-bold">Tipo Contacto
                            </th>
                            <th class="py-2 px-4 text-left text-gray-900 dark:text-gray-200 nunito-bold">Valor Contacto
                            </th>
                            <th class="py-2 px-4 text-left text-gray-900 dark:text-gray-200 nunito-bold">Cliente</th>
                            <th class="py-2 px-4 text-left text-gray-900 dark:text-gray-200 nunito-bold">Acciones</th>
                        </tr>
                        ...
                        <template x-if="loadingContactos">
                            <tr>
                                <td colspan="4" class="py-8 text-center text-gray-500 nunito-regular"><i
                                        class="fas fa-spinner fa-spin mr-2"></i> Cargando contactos...</td>
                            </tr>
                        </template>
                        <template x-if="!loadingContactos && filteredContactos().length === 0">
                            <tr>
                                <td colspan="4" class="py-8 text-center text-gray-500 nunito-regular">No se encontraron
                                    contactos.</td>
                            </tr>
                        </template>
                        ...
                        <div class="flex justify-between items-start">
                            <div>
                                ...
                                <p class="text-xs text-gray-500 dark:text-gray-400"
                                    x-text="clienteLabelById(c.id_cliente_fk) || '—'"></p>
                            </div>
                        </div>
            ...
            <div class="flex flex-col gap-4 xl:grid xl:grid-cols-2 xl:gap-6">
                ...
                <div>
                    <label for="edit_estado_solicitud"
                        class="block text-sm font-medium text-gray-700 nunito-bold">Estado de
                        la
                        Solicitud</label>
    ...
    <x-admin.edit-modal class="nunito-bold" modalName="isEditEstadoModalOpen" title="Editar Estado de Solicitud"
        ...
        <template x-if="estadoToEdit">
            <div class="flex flex-col gap-4 xl:grid xl:grid-cols-2 xl:gap-6">
                <div>
                    <label for="edit_nombre_estado" class="block text-sm font-medium text-gray-700 nunito-bold">Nombre
                        del
                        Estado</label>
