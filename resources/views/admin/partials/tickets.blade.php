<div x-data="{
  ...
  <x-responsive-table class="bg-white dark:bg-gray-900 rounded-xl shadow-lg p-4" title="Gestión de Tickets">
        ...
        <tbody>
          ...
          <template x-if="!loading && tickets.length>0">
            ...
            <template x-for="(t, index) in paginatedTickets()" :key="t.id_ticket_pk">
              <tr class="border-b border-gray-200 dark:border-gray-700 nunito-regular" :class="{ 'border-t-0': index === 0, 'last:border-b-0': index === paginatedTickets().length - 1 }">
    ...
    <x-slot name="cards">
      <template x-if="loading">
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow border border-gray-200 dark:border-gray-700 p-8 text-center text-gray-500 nunito-regular"><i class="fas fa-spinner fa-spin mr-2"></i> Cargando tickets...</div>
      </template>
      <template x-if="!loading && tickets.length===0">
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow border border-gray-200 dark:border-gray-700 p-8 text-center text-gray-500 nunito-regular">No hay tickets</div>
      </template>
      <template x-if="!loading && tickets.length>0">
          ...
          <div class="bg-white dark:bg-gray-800 rounded-lg shadow border border-black dark:border-gray-800 p-4">
            <div class="flex justify-between items-start mb-2">
              <div class="space-y-1">
                ...
                <div class="nunito-regular text-gray-900 dark:text-gray-100" x-text="t.id_ticket_pk"></div>
              </div>
            ...
            <div class="space-y-1 text-sm">
              <div><span class="font-medium text-gray-600 dark:text-gray-300 nunito-bold">Cliente:</span><span class="text-gray-900 dark:text-gray-200 nunito-regular" x-text="t.cliente_nombre"></span></div>
              <div><span class="font-medium text-gray-600 dark:text-gray-300 nunito-bold">Fecha:</span><span class="text-gray-900 dark:text-gray-200 nunito-regular" x-text="t.fecha_creacion"></span></div>
              <div><span class="font-medium text-gray-600 dark:text-gray-300 nunito-bold">Técnico:</span><span class="text-gray-900 dark:text-gray-200 nunito-regular" x-text="t.tecnico_nombre"></span></div>
              <div><span class="font-medium text-gray-600 dark:text-gray-300 nunito-bold">Descripción:</span><span class="text-gray-900 dark:text-gray-200 nunito-regular" x-text="t.descripcion_ticket"></span></div>
            </div>
            <div class="flex justify-end gap-2 mt-3 pt-3 border-t border-gray-200 dark:border-gray-700">
              <button @click="openEdit(t)" class="px-3 py-1 text-xs bg-blue-600 text-white rounded hover:bg-blue-700 flex items-center gap-1 nunito-regular"><i class="fas fa-edit"></i> Editar</button>
              <button @click="openDelete(t)" class="px-3 py-1 text-xs bg-red-600 text-white rounded hover:bg-red-700 flex items-center gap-1 nunito-regular"><i class="fas fa-trash"></i> Eliminar</button>
            </div>
          </div>
        </template>
      </template>
  ...
  <div x-show="tickets.length > perPageTickets" class="mt-6 flex flex-col items-center w-full text-gray-700 dark:text-gray-200">
    <div class="mb-2">
      <span class="inline-block text-sm text-gray-700 dark:text-gray-200 bg-white/90 dark:bg-gray-800/60 px-4 py-1 rounded-full shadow-sm">
        ...
        a
        <strong class="font-medium mx-1 text-gray-900 dark:text-white" x-text="Math.min(currentPageTickets * perPageTickets, tickets.length)"></strong>
        de
    ...
    <div class="flex items-center gap-3 bg-white border border-gray-200 p-2 rounded-lg shadow-sm dark:bg-gray-900/80 dark:border-gray-800">
      <button @click="prevPageTickets()" :disabled="currentPageTickets === 1" class="flex items-center gap-2 px-3 py-2 rounded-md text-sm font-medium transition-colors disabled:opacity-50 bg-gray-50 text-gray-700 hover:bg-gray-100 dark:bg-gray-800/60 dark:text-gray-200 dark:hover:bg-gray-800">
      ...
      <div class="flex items-center gap-1">
        <template x-for="page in Array.from({length: totalPagesTickets()}, (_, i) => i + 1).slice(Math.max(0, currentPageTickets - 3), currentPageTickets + 2)" :key="page">
          <button @click="currentPageTickets = page" class="px-3 py-1 rounded-md text-sm font-medium transition transform text-gray-700 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-gray-800" :class="page === currentPageTickets ? 'bg-blue-600 text-white' : ''">
            <span x-text="page"></span>
          </button>
        </template>
      </div>
      <button @click="nextPageTickets()" :disabled="currentPageTickets === totalPagesTickets()" class="flex items-center gap-2 px-3 py-2 rounded-md text-sm font-medium bg-blue-600 text-white hover:bg-blue-700 disabled:opacity-50 transition-colors">
        <span>Siguiente</span>
  ...
  <!-- Modal Nuevo Ticket -->
  <x-admin.form-modal class="nunito-bold" modalName="isModalOpen" title="Nuevo Ticket" submitLabel="Guardar Ticket" maxWidth="max-w-2xl" formId="form-ticket-add">
  ...
  <!-- Modal Editar Ticket -->
  <x-admin.edit-modal class="nunito-bold" modalName="isEditModalOpen" title="Editar Ticket" itemToEdit="ticketToEdit" maxWidth="max-w-2xl" formId="form-ticket-edit">
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
      <div>
        <label class="block text-sm font-medium text-gray-700 nunito-bold">Fecha</label>
  ...
  <x-admin.confirmation-modal class="nunito-bold" modalName="isDeleteModalOpen" itemToDelete="ticketToDelete" message="¿Estás seguro de que quieres eliminar el ticket?" />
