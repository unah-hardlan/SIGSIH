...
<div class="flex flex-col sm:flex-row flex-wrap gap-4 w-full items-center">
    ...
    <div class="relative w-full sm:w-auto sm:flex-grow">
        ...
        <input type="text" x-model.debounce.500ms="{{ $searchModel }}" placeholder="Buscar..."
    ...
    <div class="flex items-center gap-2 w-full sm:w-auto">
        ...
        <select x-model="{{ $ordenarModel }}"
            class="border border-gray-500 rounded px-3 py-2 text-sm font-semibold nunito-bold w-full dark:bg-gray-900 dark:border-gray-700 dark:text-gray-200 flex-grow">
        ...
        <button @click="{{ $ordenarDirectionModel }} = ({{ $ordenarDirectionModel }} === 'asc' ? 'desc' : 'asc')"
            ...
            <i class="fas"
                :class="{ 'fa-sort-up': {{ $ordenarDirectionModel }} === 'asc', 'fa-sort-down': {{ $ordenarDirectionModel }} === 'desc' }"></i>
        </button>
        @endif
